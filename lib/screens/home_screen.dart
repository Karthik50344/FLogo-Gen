...
import '../widgets/about_contact_teaser.dart';
                  ...
                  Expanded(
                    child: LayoutBuilder(
                      builder: (context, outerConstraints) {
                        // No max-width cap: content fills the available
                        // screen width. Padding scales up a bit on larger
                        // screens so cards don't run edge-to-edge on huge
                        // monitors.
                          ...
                          child: Column(
                            children: [
                              ...
                              LayoutBuilder(builder: (context, constraints) {
                                ...
                                // Width each card actually gets once split
                                // into the two-column layout, so the
                                // grid/field column counts below match
                                // reality instead of guessing off the full
                                // row width.
                                final cardWidth = wide
                                    ? (constraints.maxWidth - 20) / 2
                                    : constraints.maxWidth;

                                // Always 2 rows of 3 (never collapse to a
                                // single row of 6, even on wide screens) —
                                // only drop to 2-per-row on genuinely
                                // narrow cards.
                              ...
                              // Long-form landing-page content below the
                              // tool itself — capped to a readable width
                              // even though the generator UI above uses
                              // the full screen.
                              Center(
